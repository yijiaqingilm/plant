...
var userApi = {
   ...
   */
  booking (user) {
    return common.postRequest('/customer/main/booking', user, {isManager: true})
  },
  /**
   * 重新预约时间
   * @returns {*}
   ...
   */
  payment (order) {
    return common.postRequest('/customer/main/payment', order, {isManager: true})
  },
  /**
   * 更多套餐接口
   * @param type
   ...
   * @return {*}
   */
  couponList (cartids, paytype = '', paymethod = '') {
   ...
   */
  categoryList () {
    return common.getRequest('/category/list', {}, {})
  },
  /**
   * 获取商品分类详情接口，商品分类详情包含了商品分类轮播图列表，以及商品列表
   * @param id
   * @return {*}
   */
  categoryItemsById (id, page) {
    return common.getRequest('/category/item-detail', {id, page}, {})
  },
  /**
   * 获取套餐分类详情接口，套餐分类详情包含了套餐分类轮播图列表，以及套餐列表
   * @param id
   */
  categoryCompositeById (id, page) {
    return common.getRequest('/category/composite-detail', {id, page}, {})
  },
  /**
   * 优惠礼包信息接口
   * @return {*}
   */
  getPackage () {
    return common.getRequest('/coupon/get-package')
  },
  /**
   * 优惠礼包获取接口
   * @return {*}
   */
  getMyCoupon () {
    return common.getRequest('/coupon/get-my-coupon')
  },
  /**
   * 关闭公告
   * @return {*}
   */
  closeNotice () {
    return common.getRequest('/user/set-notice')
  },
  /**
   * 获取Banner接口
   * @return {*}
   */
  bannerList (view) {
    return common.getRequest('/banner/index', {
      view
    })
  }
