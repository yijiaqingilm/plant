        ...
        <div class="list-block list-block-search searchbar-found no-hairlines">
            <ul class="search-list ">
                <li v-for="(data,index) in dataList" @click="goSearch(data)">
                    <div>{{data}}</div>
                    <div @click.stop="delSearchHistory(index)"><img src="../../assets/icon_x.png" class="icon_del"
                                                                    alt="">
                    </div>
                </li>
            </ul>
...
<script type="text/ecmascript-6">
    ...
    export default {
        ...
        created() {
            this.dataList = LocalCache.get('searchHistory') || [];
            window.wx.ready(() => {
                wx.hideMenuItems({
        ...
        methods: {
            clearHistory() {
                this.dataList = [];
                LocalCache.del("searchHistory");
            },
            delSearchHistory(index) {
                this.dataList.splice(index, 1);
                LocalCache.set('searchHistory', this.dataList);
            },
            reinit: function () {
                this.dataList = LocalCache.get('searchHistory') || [];
            },
            search: function () {
                if (this.query.key.length > 0) {
                    ...
                    LocalCache.set('searchHistory', Array.from(setDataList));
                }
                this.goSearch(this.query.key);

            },
            goSearch(key) {
                this.$router.load({url: '/sell/search', query: {key}});
            },
            loadData: function () {
            }
        },
