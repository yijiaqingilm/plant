            ...
            <div class="address-list">
                <div class="address-item" v-for="(address,index) in addressList" @click="saveAddress(address)">
                    ...
                    <div class="set">
                        ...
                        <div v-if="address.ismodify" class="edit" @click.stop="toEditAddress(address)"><span class="icon_edit"></span>编辑</div>
                        <div v-if="address.ismodify" class="del" @click.stop="delAddress(address)"><span class="icon_del"></span>删除</div>
                    </div>
                </div>
            </div>
        </f7-block>
            ...
            delAddress: function (address) {
                ...
                this.$f7.confirm('', '确定删除？', () => {
                    ...
                    this.$store.dispatch("removeAddressByIdAsync", {
                        ...
                        customer_id,
                        callBack: function () {
                            if (that.$store.state.default_address.id == address.id) {
