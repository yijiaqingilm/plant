            ...
            <div class="content" v-if="orderInfo!=null">
                ...
                <div class="goods-wrap" v-if="orderInfo.leases&&orderInfo.leases.items.length>0">
                    ...
                    <div class="order-info-group">
                        <div>结算方式</div>
                        <div>
                            <span>{{payTypeToName[payInfo.payType].name}}</span>
                        </div>
                    </div>
                    <div class="order-info-group" @click="alertDeposit">
                        <div>租赁押金<img src="../../assets/icon_issue.png" alt="" class="icon_issue"></div>
                        <div>￥{{depositAll}}</div>
                    </div>
                    <div class="order-info-group">
                        <div>租赁金额</div>
                ...
                <div class="line-10"></div>
                <div class="order-info-group" @click="alertShipfee">
                    <div>配送及人工费用<img src="../../assets/icon_issue.png" alt="" class="icon_issue"></div>
                    <div>￥{{shipfeeTotal }}</div>
                </div>
                <div class="order-info-group" v-if="orderInfo.leases&&orderInfo.leases.items.length>0">
                    <div class="color-theme">
                        <my-checkbox id="agreement" v-model="check"></my-checkbox>
                        <span @click="$router.load({url:'/user/agreement'})">合同协议</span>
                    </div>
                </div>
            </div>
        </f7-block>
...
<script type="text/ecmascript-6">
  ...
  import { payType, timeType, weekList, goodsType, itemType, payTypeToName } from 'lib/common'
  ...
  export default {
    ...
    methods: {
      ...
      goPay: function () {
        ...
        this.$router.load({url: '/store/confirm/order'})
